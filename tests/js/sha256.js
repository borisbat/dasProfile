...
function digestBlock(msg, index, hash) {
  const digest = new Array(64).fill(0);
  for (let j = 0; j < 16; ++j) {
    digest[j] = readInt32(msg, index + j * 4);
  }
  for (let j = 16; j < 64; ++j) {
    const v0 = digest[j - 15];
    const s0 = (rrotate(v0, 7) ^ rrotate(v0, 18) ^ (v0 >>> 3)) >>> 0;
    const v1 = digest[j - 2];
    const s1 = (rrotate(v1, 17) ^ rrotate(v1, 19) ^ (v1 >>> 10)) >>> 0;
    digest[j] = (digest[j - 16] + s0 + digest[j - 7] + s1) >>> 0;
  }

  let [a, b, c, d, e, f, g, h] = hash;
  for (let i = 0; i < 64; ++i) {
    const s0 = (rrotate(a, 2) ^ rrotate(a, 13) ^ rrotate(a, 22)) >>> 0;
    const maj = ((a & b) ^ (a & c) ^ (b & c)) >>> 0;
    const t2 = (s0 + maj) >>> 0;
    const s1 = (rrotate(e, 6) ^ rrotate(e, 11) ^ rrotate(e, 25)) >>> 0;
    const ch = ((e & f) ^ (~e & g)) >>> 0;
    const t1 = (h + s1 + ch + primes[i] + digest[i]) >>> 0;
